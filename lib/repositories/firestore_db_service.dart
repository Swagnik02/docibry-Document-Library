...
class FirestoreDbService {
  ...
  Future<void> addDocumentFromFirestore(String userEmail, DocModel doc) async {
    try {
      DocumentReference userDocRef =
          _firestore.collection('users').doc(userEmail.toString());
  ...
  Future<void> updateDocumentFromFirestore(
      String userEmail, DocModel doc) async {
    try {
      DocumentReference userDocRef =
          _firestore.collection('users').doc(userEmail.toString());
  ...
  Future<void> deleteDocumentFromFirestore(String userEmail, String uid) async {
    try {
      DocumentReference userDocRef =
          _firestore.collection('users').doc(userEmail.toString());
